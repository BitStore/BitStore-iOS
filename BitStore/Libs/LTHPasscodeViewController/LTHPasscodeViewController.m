...
- (void)showLockScreen:(UIViewController *)vc animated:(BOOL)a {
	_displayedAsModal = YES;
    self.title = NSLocalizedString(@"unlock", nil);
    [self _prepareNavigationControllerWithController:vc a:a];
}

- (void)_prepareNavigationControllerWithController:(UIViewController *)viewController {
    [self _prepareNavigationControllerWithController:viewController a:YES];
}
