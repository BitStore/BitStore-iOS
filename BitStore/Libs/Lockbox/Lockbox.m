...
+(NSString *)objectForKey:(NSString *)key sync:(BOOL)sync
{
    NSString *hierKey = [self _hierarchicalKey:key];
    ...
    [query setObject:hierKey forKey: (LOCKBOX_ID)kSecAttrService];
    if (sync) {
        [query setObject: (LOCKBOX_ID) kCFBooleanTrue forKey: (LOCKBOX_ID) kSecAttrSynchronizable];
    }
...
+ (BOOL)setString:(NSString *)value forKey:(NSString *)key {
    return [self setObject:value forKey:key accessibility:DEFAULT_ACCESSIBILITY];
}

+ (NSString *)stringForKey:(NSString *)key {
    return [self objectForKey:key sync:NO];
}
...
+(NSArray *)arrayForKey:(NSString *)key {
    return [self arrayForKey:key sync:NO];
}

+(NSArray *)arrayForKey:(NSString *)key sync:(BOOL)sync
{
    NSArray *array = nil;
    NSString *components = [self objectForKey:key sync:sync];
    if (components)
...
+ (NSDictionary *)dictionaryForKey:(NSString *)key {
    return [self dictionaryForKey:key sync:NO];
}

+ (NSDictionary *)dictionaryForKey:(NSString *)key sync:(BOOL)sync
{
    NSArray * keysAndValues = [self arrayForKey:key sync:sync];
